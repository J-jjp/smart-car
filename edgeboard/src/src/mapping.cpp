...
public:
    ...
    {
        // Generate IPM image from src
        remap(_inputImg, _dstImg, m_mapX, m_mapY, INTER_LINEAR); //, BORDER_CONSTANT, Scalar(0,0,0,0));
    }
...
private:
    ...
    // Points
    std::vector<cv::Point2f> m_origPoints;//Point2f(float _x = 0, float _y = 0) : x(_x), y(_y) {}
